	...
	try:
		...
		for (lpBox, chars) in plates:
			...
			for char in chars:
					...
					if chr(key) not in '0123456789':
						hangul = chr(key) + chr(cv2.waitKey(0))
						dirPath = "{}/{}".format(args["examples"], hangul)
					else:
						dirPath = "{}/{}".format(args["examples"], chr(key))
					...
					path = "{}/{}.jpg".format(dirPath, str(count).zfill(5))
