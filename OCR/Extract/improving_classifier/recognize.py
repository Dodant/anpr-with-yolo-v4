# USAGE
# python recognize.py --images /Users/Dodanto/Downloads/신규번호판/all --char-classifier output/adv_char.cpickle --digit-classifier output/adv_digit.cpickle
...
for imagePath in sorted(list(paths.list_images(args["images"]))):
	...
	lpd = LicensePlateDetector(image, numChars=8)
	plates = lpd.detect()
	...
	for (lpBox, chars) in plates:
		...
		cv2.putText(image, text, (cX - (cX // 5), cY - 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
